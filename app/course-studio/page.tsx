    ...
    <div className="max-w-[820px]">
      <section className="editorial-card mb-5 border-[var(--amber-tint)] bg-status-warn-soft px-4 py-4 md:hidden">
        ...
        <p className="mt-2 text-sm leading-relaxed text-ink-muted">
          You can use Brightspace Manager on a phone to check status, confirm whether a course is
          live, or inspect read-only details. Building and editing courses needs the desktop Studio
          layout.
        </p>
      ...
      <div className="hidden md:block">
        <header className="mb-[22px]">
          ...
          <p className="mt-[5px] max-w-[680px] text-sm leading-[1.55] text-ink-muted">
            Build Brightspace-ready courses from our shared template. Course Studio walks you
            through each step. Write in plain English, export when you&apos;re done.
          </p>
        </header>
            ...
            <>
              ...
              <div className="mb-2.5 flex flex-wrap gap-x-[18px] gap-y-1.5">
                {template.files.map((file) => (
                  <span
                    key={file.name}
                    className="inline-flex items-center gap-[5px] text-xs text-ink-muted"
                  >
                    <svg
                      width="12"
                      height="12"
              ...
              <p className="text-[12.5px] leading-[1.55] text-ink-soft">
                Every course you build pulls from this template automatically, so the look and feel
