...
import { isDeployReady, lmsFileUrl } from "@/lib/studio/deploy";
import { readImage } from "@/lib/studio/images";
      ...
      const topics = mod.topics
        .map(
          ...
          file: ${jsString(`${t.slug}.html`)},${
            lms
              ? `\n          url: ${jsString(lmsFileUrl(publish!, `${t.slug}.html`))},`
 ...
 *
 * DEPLOY-READY: every URL below is pre-filled for Brightspace org unit
 * ${publish!.orgUnitId} (${publish!.orgUnitCode.trim()}). Upload every file in this
 * package to ${publish!.folderPath.trim() ? `the "${publish!.folderPath.trim()}" folder of` : "the ROOT of"} Manage Files — nothing needs editing.
 */`
    : `/**
 * Course Configuration — single source of truth for the LACE wrapper.
 * Generated by Brightspace Manager Course Studio. Safe to edit by hand.
 ...
 */`;
  ...
  const targets = lms
    ? `  courseHomeUrl: ${jsString(lmsFileUrl(publish!, "Home.html"))},
  completeUrl: ${jsString(lmsFileUrl(publish!, "complete.html"))},`
...
window.COURSE_CONFIG = {
  deployMode: ${jsString(lms ? "lms" : "local")},
...
function accordionBlock(items: { title: string; body: string }[], reveal: boolean): string {
  ...
  const details = rows
    .map(
      (
        it,
      ) => `      <details class="${cls}-item" style="border:1px solid var(--line,#e4e7ed);border-radius:10px;margin-bottom:.55rem;overflow:hidden;">
  ...
  const tone =
    block.variant === "warn"
      ...
      : block.variant === "tip"
        ...
        : {
            accent: "var(--accent,#1c3fb0)",
            border: "var(--line,#cdd5e4)",
            bg: "var(--accent-tint,#eef2fb)",
            label: "Note",
          };
...
/* Plain-language upload companion for deploy-ready packages: where the files
   go and the final URL for each page, ready to paste into Content topics. */
export function generateDeployTxt(draft: CourseDraft): string {
  const publish = draft.publish!;
  const folder = publish.folderPath.trim().replace(/^\/+|\/+$/g, "");
  const lines: string[] = [
    `HOW TO PUBLISH "${draft.courseTitle}"`,
    "",
    `1. In Brightspace, open course ${publish.orgUnitId} (${publish.orgUnitCode.trim()})`,
    "   and go to Course Admin -> Manage Files.",
    folder
      ? `2. Upload EVERY file in this package into the "${folder}" folder`
      : "2. Upload EVERY file in this package to the ROOT of Manage Files",
    "   (keep the images/ folder together with the pages).",
    "3. In Content, create one topic per page below and paste its URL.",
    "   Nothing in this package needs editing.",
    "",
    "PAGE URLS",
    "",
    `Course home:  ${lmsFileUrl(publish, "Home.html")}`,
    `Completion:   ${lmsFileUrl(publish, "complete.html")}`,
    "",
  ];
  for (const mod of draft.modules) {
    ...
    for (const t of mod.topics) {
      ...
      lines.push(`  ${lmsFileUrl(publish, `${t.slug}.html`)}`);
    }
    lines.push("");
  }
  return lines.join("\n");
...
export async function buildPackageFiles(
  ...
  if (isDeployReady(draft)) {
    files.set("DEPLOY.txt", generateDeployTxt(draft));
  }
  ...
  /* Uploaded images ship inside the package; anything still referenced by
     name only gets called out so it isn't silently missing in Brightspace. */
  const missing: string[] = [];
  for (const name of referencedImages(draft)) {
    const image = await readImage(draft.id, name);
    if (image) files.set(`images/${name}`, new Uint8Array(image.bytes));
    else missing.push(name);
  }
  if (missing.length > 0) {
    files.set(
      "images/README.txt",
      "These images are referenced in the course but were never uploaded\n" +
        "in the Studio. Drop each file into this folder before uploading\n" +
        "to Brightspace:\n\n" +
