  ...
  if (drafts.length === 0) {
    return (
      <p className="px-5 py-5 text-[13px] text-ink-muted">
        No courses yet. Hit <strong>+ New course</strong> to open a blank builder for a guided form
        with a live preview of what learners will see.
      </p>
    );
  }
  ...
  return (
    <>
      {drafts.map((draft) => (
        ...
        >
          <Link
            ...
            className="flex min-w-0 flex-1 items-center gap-[13px]"
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="block text-xs text-ink-soft">
                {draft.topicCount} lesson{draft.topicCount === 1 ? "" : "s"} · {draft.totalMinutes}{" "}
                min · edited {formatRelative(draft.updatedAt)}
              </span>
            </span>
          </Link>
          {draft.deployReady ? (
            <StatusBadge tone="ok">deploy-ready</StatusBadge>
          ) : (
      ...
      <ConfirmDialog
        ...
        message={
          <>
            This permanently removes the course and its {pending?.topicCount ?? 0} lesson
            {pending?.topicCount === 1 ? "" : "s"}. This can&apos;t be undone.
